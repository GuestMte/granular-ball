...
#98.3 8个属性 1.4 0.99 [3, 12, 0, 9, 8, 5, 2,]
	...
	def init_granular_balls(self,er):
		...
		while True:#先对原有颗粒球进行分裂，然后再对分裂后的颗粒球进行检查是否需要分裂，直到所有颗粒球都满足条件，即i(当前颗粒球的序号)>ll(当前颗粒球的数量)
			if self.granular_balls[i].SD > er :#如果选择的颗粒球纯度小于阈值，并且数量大于最小样本
				split_balls = self.granular_balls[i].split_2balls(number=number)#对颗粒球进行分裂，返回的是分裂后的颗粒球
...
def attribute_reduce(data,number):
	print('总数量',number)
	#1.2 wine 97.77
	er=1.4
	len_sum = 9999
	...
	while len(re_attribu):#遍历所有属性
		...
		for i in re_attribu:#依此添加属性
			...
			for ball in ball_list1:#对颗粒球列表遍历
				if ball.purity >= 0.99:  # 如果颗粒球的纯度大于等于1
		...
		# 	ball_len=len(ball_list1)
		# 	print('小球的数量',ball_len)
		# 	if ball_len < N_bal_len:  #
		# 		N_bal_len = ball_len  # 当前正域赋值给Pos_num
		# 		N_i = i  # 属性列表
		# if N_bal_len==1:
		# 	print('N_i',N_i)
		# 	print('re_attribute',re_attribu)
		# 	re_attribu.remove(N_i)
		# 	print('一个球的数量', len(ball_list1))
		# elif N_bal_len <= len_sum:  #
		# 	len_sum = N_bal_len  # 当前正域赋值给Pos_num
		# 	attribu.append(N_i)  # 属性列表
		# 	re_attribu.remove(N_i)
		# 	print('球的数量',len(ball_list1))
		else:
			return attribu
	return attribu
...
if __name__ == "__main__":
    datan = ["\wine1"]
    desired_cols =['Flavanoids', 'wines', 'Color', 'Alcohol', 'Total', 'Proline', 'Proanthocyanins', 'Nonflavanoid',
    ...
    for name in datan:
        ...
        if 'class' in df.columns:
			# 将 'class' 列添加到 cols_to_sort 列表的末尾
            desired_cols.append('class')
            df = df[desired_cols]
        ...
        Row = attribute_reduce(data_U,number)
        #print('Row',Row)
        mat_data = U[:, Row]
        ...
        end_time=time.time()
        print("pre", avg, std,Row,"time",end_time-start_time)

        # for i in len(re_attribu):
		#
		# 	gb = GBList(data_U, re_attribu)#构建粒球对象
		# 	gb.init_granular_balls()
		# 	ball_list1 = gb.granular_balls
		# 	print('len',len(ball_list1))
		# 	for z in ball_list1:
		# 		print('各粒球SD的值',z.SD)
		# 	print('粒球数量', len(ball_list1))
